...
class SDKSettings(BaseSettings):
    server: str = Field(
        default=None,
        description=format_docstring("""The PI server name to connect to through
    ...
    path: FilePath = Field(
        ...
        description=format_docstring("""The absolute path to the directory
        containing the SDK assembly files. Defaults to
        'C:/Program Files/PIPC/pisdk/PublicAssemblies/OSIsoft.PISDK.dll'.""")
    )
    max_connections: int = Field(
        default=4,
        description=format_docstring("""The maximum number of connections that
...
class WebSettings(BaseSettings):
    host: str = Field(
        default=None,
        description=format_docstring("""The host to connect to the web API.
        Defaults to `None`. This is required for any web API queries.""")
    )
    port: int = Field(
        default=None,
        description=format_docstring("""The port to connect to the web API.
        Defaults to `None`, if TLS is `True` will use 443 else 80.""")
    )
    tls: bool = Field(
        default=False,
        description=format_docstring("""Connect to the web API over TLS. Defaults
        to `False`.""")
    )
    ...
    verify: bool | FilePath = Field(
        default=True,
        description=format_docstring("""Either a boolean, in which case it
    ...
    headers: Dict[str, str] = Field(
        default=None,
        description=format_docstring("""Dictionary of HTTP Headers to send with
        each request. Defaults to `None`.""")
    )
    cookies: Dict[str, str] = Field(
        default=None,
        description=format_docstring("""Dictionary of cookies to send with each
        request. Defaults to `None`.""")
    )
    kerberos: bool = Field(
        default=True,
        description=format_docstring("""If `True`, kerberos authentication will
        used on requests. Defaults to `True`.""")
    )
    mutual_authentication: int = Field(
        default=REQUIRED,
        decription=format_docstring("""Integer value defining the mutual
        authentication requirements (1: Required, 2: Optional, 3: Disabled).
        Defaults to 1 (Required).""")
    )
    delegate: bool = Field(
        default=False,
        description=format_docstring("""Indicates that the user's credentials
        are to be delegated to the server. Defaults to `False`.""")
    )
    service: str = Field(
        default="HTTP",
        description=format_docstring("""The service principle protocol to use
        (such as HTTP or HTTPS) for kerberos authentication. Defaults to 'HTTP'.""")
    )
    principal: SecretStr = Field(
        default=None,
        description=format_docstring("""An explicit principal to use for
        authentication. By default the user running the application is used.
        Defaults to `None`.""")
    )
    hostname_override: str = Field(
        default=None,
        description=format_docstring("""If communicating with a host whose DNS
    ...
    send_cbt: bool = Field(
        default=True,
        description=format_docstring("""If `True`, automatically attempts to
    ...
    def kerberos_settings(self) -> Dict[str, str | bool | int | None]:
        ...
        return {
            ...
            "principal": self.principal if self.principal is None else self.principal.get_secret_value(),
